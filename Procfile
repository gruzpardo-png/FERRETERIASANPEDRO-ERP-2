...
APP_VERSION = "v4.15 Despacho Único y PDF Brand"
...
@app.template_filter("gross_cost")
def gross_cost(value):
    try:
        number = float(value or 0)
    except Exception:
        number = 0
    return number * (1 + iva_rate())
...
def init_db():
    ...
    execute("CREATE INDEX IF NOT EXISTS idx_despachos_tipo_numero ON despachos(tipo_documento, numero_documento)")
    ...
    execute("""
    CREATE TABLE IF NOT EXISTS cotizador_sesiones (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        numero_pdf TEXT,
        cliente TEXT,
        contacto TEXT,
        ...
        archivo_nombre TEXT,
        texto_pdf TEXT,
        total_neto_pdf REAL DEFAULT 0,
        iva_pdf REAL DEFAULT 0,
        total_bruto_pdf REAL DEFAULT 0,
        total_bruto_actual REAL DEFAULT 0,
        contribucion_actual REAL DEFAULT 0,
        margen_actual_pct REAL DEFAULT 0,
        total_bruto_nuevo REAL DEFAULT 0,
        contribucion_nueva REAL DEFAULT 0,
        margen_nuevo_pct REAL DEFAULT 0,
        costo_total_venta REAL DEFAULT 0,
        validez_oferta_dias INTEGER DEFAULT 2,
    ...
    CREATE TABLE IF NOT EXISTS cotizador_items (
        ...
        descripcion TEXT,
        cantidad REAL DEFAULT 1,
        precio_unitario_bruto_pdf REAL DEFAULT 0,
        valor_total_linea_pdf REAL DEFAULT 0,
        descuento_pct REAL DEFAULT 0,
        precio_unitario_bruto_final REAL DEFAULT 0,
        costo_neto REAL DEFAULT 0,
        stock REAL DEFAULT 0,
        encontrado INTEGER DEFAULT 0,
        margen_pct REAL DEFAULT 0,
        contribucion_unitaria REAL DEFAULT 0,
        contribucion_total REAL DEFAULT 0,
        total_linea_bruto_final REAL DEFAULT 0,
        observacion TEXT,
        created_at TEXT NOT NULL,
        updated_at TEXT
    )
    ...
    execute("CREATE INDEX IF NOT EXISTS idx_cotizador_items_sesion ON cotizador_items(sesion_id)")
    execute("CREATE INDEX IF NOT EXISTS idx_cotizador_sesiones_created ON cotizador_sesiones(created_at)")
...
PERMISSION_LABELS = {
    "ventas": "Ventas / Cotización Elias",
    "cotizador": "Ventas / Cotizador PDF",
    "cotizador_exportar": "Cotizador / PDF y Excel",
    "cotizador_precios": "Cotizador / Modificar precios y descuentos",
    "productos": "Productos / Ver maestro de precios",
    "productos_importar": "Productos / Importar maestro Excel",
    "despachos": "Despachos / Crear",
    "despachos_editar": "Despachos / Editar",
    "mantenciones": "Mantenciones",
    "consulta": "Consulta operación",
...
def is_superior(user=None):
    """
    Permiso superior exclusivo.
    Por instrucción de gerencia, solo el usuario gus tiene estas atribuciones.
    No se hereda por rol admin.
    """
    user = user or current_user()
    if not user:
        return False
    return str(user["username"] or "").strip().lower() == "gus"
...
def has_any_perm(*module_names):
    return any(has_perm(m) for m in module_names)


def permission_any_required(*module_names):
    def decorator(fn):
        @wraps(fn)
        def wrapper(*args, **kwargs):
            if not current_user():
                return redirect(url_for("login"))
            if not has_any_perm(*module_names):
    ...
    <style>
        ...
        *{box-sizing:border-box}
        ...
        .flash.critical{
            ...
            box-shadow:0 12px 28px rgba(153,27,27,.30);
            font-size:17px;
            padding:18px 20px;
            text-transform:uppercase;
        }
        ...
        .excel-table input[type=number]{text-align:right;min-width:105px}
        .excel-desc{min-width:380px;min-height:42px}
        .calc-toolbar{
            display:grid;
            grid-template-columns:repeat(4,minmax(0,1fr));
            gap:12px;
        ...
        .metric-positive{color:#047857;font-weight:900}
        .metric-warning{color:#b45309;font-weight:900}
        .metric-bad{color:#b91c1c;font-weight:900}
        .cotizador-badge{
            display:inline-flex;
            padding:4px 9px;
            border-radius:999px;
        ...
        .cotizador-badge.ok{background:#d1fae5;color:#065f46}
        .cotizador-badge.warn{background:#fef3c7;color:#92400e}
        .cotizador-badge.bad{background:#fee2e2;color:#991b1c}
        .cotizador-analisis-table{
            table-layout:fixed;
            min-width:1260px;
        }
        .cotizador-analisis-table th,
        .cotizador-analisis-table td{
            font-size:10.5px;
            padding:7px 6px;
        ...
        .cotizador-analisis-table .num{
            text-align:right;
            white-space:nowrap;
            font-weight:800;
        }
        .cotizador-analisis-table .truncate{
            overflow:hidden;
            text-overflow:ellipsis;
            white-space:nowrap;
        }
        .cotizador-analisis-table .actions{
            gap:4px;
            justify-content:flex-start;
        }
        .cotizador-analisis-table .btn-small{
            padding:6px 7px;
            font-size:10px;
        }
        .superior-note{
...
{% else %}
    <div class="topbar">
        ...
        <div class="nav-wrap">
            ...
            {% if has_perm("ventas") or has_perm("cotizador") or has_perm("productos") or has_perm("productos_importar") %}
            <div class="nav-section ventas">
                <span class="nav-label">Ventas</span>
                {% if has_perm("ventas") %}<a class="nav-link" href="{{ url_for('ventas') }}">Cotización Elias</a>{% endif %}
                {% if has_perm("ventas") or has_perm("cotizador") %}<a class="nav-link" href="{{ url_for('cotizador') }}">Cotizador</a>{% endif %}
                {% if has_perm("productos") or has_perm("productos_importar") %}<a class="nav-link" href="{{ url_for('productos') }}">Productos</a>{% endif %}
            </div>
            {% endif %}

            {% if is_admin or is_superior %}
...
def despachos():
    ...
    if request.method == "POST":
        ...
        if not numero or not tipo:
            ...
            existente = query_one("""
                SELECT * FROM despachos
                WHERE TRIM(numero_documento) = ? AND TRIM(tipo_documento) = ?
                ORDER BY id DESC
                LIMIT 1
            """, (numero, tipo))
            ...
            if existente:
                ...
                mensaje = (
                    f"ALERTA CRÍTICA: EL DOCUMENTO {tipo} {numero} YA FUE REGISTRADO "
                    f"CON ESTADO {estado_existente}. ID #{existente['id']} · "
                    f"USUARIO: {existente['usuario_nombre'] or '-'} · FECHA: {existente['created_at'] or '-'}. "
                    "NO SE PERMITE DUPLICAR LA ENTREGA."
                )
                flash(mensaje, "critical")
                write_audit("despachos", "intento_duplicado", existente["id"], "numero_documento", numero, mensaje)
                return redirect(url_for("despachos"))
    ...
    <div class="card">
        ...
        <div class="placeholder" style="margin-bottom:14px;">
            <b>Control anti-duplicado:</b> si el documento ya fue registrado, el sistema bloqueará el ingreso y mostrará una alerta crítica.
        </div>
        <form method="post">
            <div class="form-row">
                <div>
...
def consulta():
    ...
    """, rows=rows, estados=estados, request=request, is_admin=is_admin(), is_superior=is_superior())
...
def mantenciones():
    ...
    """, maqs=maqs, estados=estados, tipos=tipos, rows=rows, today=today_str(), user=current_user(), is_admin=is_admin(), is_superior=is_superior())
...
def usuarios():
    ...
    <div class="superior-note">
        <b>Permiso superior exclusivo:</b> el usuario <b>gus</b> tiene atribuciones superiores que no hereda el administrador.
        Actualmente solo gus puede eliminar análisis del Cotizador.
    </div>

    <div class="card">
        <h2>Crear usuario</h2>
        <p class="muted">Permisos ampliados para separar operación, ventas, cotizador, productos, control y administración.</p>
        <form method="post">
            <div class="form-row">
        ...
        <div class="table-wrap">
            <table>
                ...
                <tr>
                    ...
                    <td>{{ u.full_name }}</td>
                    <td>
                        {% if u.username|lower == 'gus' %}
                            <span class="badge warn">superior</span>
                        {% else %}
                            <span class="badge {% if u.role == 'admin' %}ok{% else %}info{% endif %}">{{ u.role }}</span>
                        {% endif %}
                    </td>
...
def edit_usuario(user_id):
    ...
    <div class="card">
        <form method="post">
            ...
            <h3>Permisos</h3>
            {% if u.username|lower == 'gus' %}
                <div class="superior-note"><b>Usuario superior:</b> gus conserva permisos exclusivos aunque no estén disponibles para administradores.</div>
            {% endif %}
            <div class="checkbox-grid">
                {% for key, label in permission_labels.items() %}
...
# ============================================================
# COTIZADOR PDF - VENTAS
# ============================================================

def cotizador_parse_money(value):
    return parse_number(value, 0)


def cotizador_format_number(value):
    try:
...
def extraer_texto_pdf_cotizacion(file_storage):
    """Extrae texto de PDF de cotización usando pypdf."""
...
def parsear_cabecera_cotizacion_pdf(texto):
    numero = ""
    fecha = ""
    contacto = ""
    cliente = ""

    m = re.search(r"N[º°]?\s*(\d+)", texto or "", flags=re.IGNORECASE)
    if m:
    ...
    m = re.search(r"Raz[oó]n Social\s*:\s*([^\n]+)", texto or "", flags=re.IGNORECASE)
    if m:
        cliente = m.group(1).strip()

    return {"numero": numero, "fecha": fecha, "contacto": contacto, "cliente": cliente}


def cotizador_lineas_productos_pdf(texto):
    lines = [re.sub(r"\s+", " ", l).strip() for l in (texto or "").splitlines()]
    lines = [l for l in lines if l]
    start = None
    end = None
    for idx, line in enumerate(lines):
        norm = normalize_text(line)
        if "CODIGO" in norm and "DESCRIPCION" in norm and "CANTIDAD" in norm and "PRECIO" in norm:
            start = idx + 1
            break
    if start is None:
        return []
    for idx in range(start, len(lines)):
        ...
        if "DOCUMENTO CREADO" in norm or "EXENTO" in norm or "CONDICIONES COMERCIALES" in norm:
            end = idx
            break
    return lines[start:end] if end is not None else lines[start:]


def cotizador_parse_product_buffer(buffer):
    buffer = re.sub(r"\s+", " ", buffer or "").strip()
    # Código descripción cantidad precio_unitario valor
    money = r"\d{1,3}(?:\.\d{3})*,\d{2}|\d+,\d{2}"
    pattern = re.compile(
        rf"^(?P<codigo>[A-Za-z0-9]{{3,}})\s+(?P<desc>.+?)\s+(?P<cant>{money})\s+(?P<unit>{money})\s+(?P<valor>{money})$"
    )
    ...
    return {
        ...
        "cantidad": cotizador_parse_money(m.group("cant")),
        "precio_unitario_bruto": cotizador_parse_money(m.group("unit")),
        "valor_total_linea": cotizador_parse_money(m.group("valor")),
    }


def parsear_productos_cotizacion_pdf(texto):
    lineas = cotizador_lineas_productos_pdf(texto)
    productos = []
    buffer = ""
    for line in lineas:
        if not line:
            continue
        starts_code = bool(re.match(r"^[A-Za-z0-9]{3,}\s+", line))
        if starts_code and buffer:
            parsed_prev = cotizador_parse_product_buffer(buffer)
            if parsed_prev:
                productos.append(parsed_prev)
                buffer = line
            else:
                buffer += " " + line
        else:
        ...
        parsed = cotizador_parse_product_buffer(buffer)
        if parsed:
...
def parsear_totales_cotizacion_pdf(texto):
    total_neto = iva = total_bruto = 0.0
    seccion = texto or ""
    if "Documento creado" in seccion:
        seccion = seccion.split("Documento creado", 1)[1]
    if "Exento:" in seccion:
        seccion = seccion.split("Exento:", 1)[0]
    nums = re.findall(r"\d{1,3}(?:\.\d{3})*,\d{2}|\d+,\d{2}", seccion)
    valores = [cotizador_parse_money(n) for n in nums]
    if len(valores) >= 3:
        total_neto, iva, total_bruto = valores[-3], valores[-2], valores[-1]
    return {"neto": total_neto, "iva": iva, "bruto": total_bruto}


def cotizador_calculos_linea(cantidad, precio_bruto, costo_neto):
    """
    Fórmulas del cotizador:
    - margen producto = PV bruto final / (costo neto * 1,19) - 1
    - contribución unitaria = PV bruto final / 1,19 - costo neto
    - contribución total línea = cantidad * contribución unitaria
    - total línea bruto = cantidad * PV bruto final
    """
    cantidad = cotizador_format_number(cantidad)
    precio_bruto = cotizador_format_number(precio_bruto)
    costo_neto = cotizador_format_number(costo_neto)
    venta_neta = precio_bruto / (1 + iva_rate()) if precio_bruto else 0.0
    contrib_unit = venta_neta - costo_neto
    contrib_total = contrib_unit * cantidad
    total_bruto = precio_bruto * cantidad
    margen = (precio_bruto / (costo_neto * (1 + iva_rate())) - 1) if costo_neto > 0 else 0.0
    return {
        "margen_pct": margen,
        "contribucion_unitaria": contrib_unit,
        "contribucion_total": contrib_total,
        "total_linea_bruto_final": total_bruto,
    }


def cotizador_totales_desde_items(items):
    total_bruto = 0.0
    costo_neto_total = 0.0
    contrib_total = 0.0
    for i in items:
        cantidad = float(i["cantidad"] or 0)
        precio = float(i["precio_unitario_bruto_final"] or 0)
        costo = float(i["costo_neto"] or 0)
        total_bruto += precio * cantidad
        costo_neto_total += costo * cantidad
        contrib_total += ((precio / (1 + iva_rate())) - costo) * cantidad
    margen = (total_bruto / (costo_neto_total * (1 + iva_rate())) - 1) if costo_neto_total > 0 else 0.0
    return {
        "total_bruto": total_bruto,
        "costo_neto_total": costo_neto_total,
        "contribucion": contrib_total,
        "margen": margen,
    }


def cotizador_recalcular_sesion(sesion_id):
    items = query_all("SELECT * FROM cotizador_items WHERE sesion_id = ? ORDER BY id", (sesion_id,))
    totals = cotizador_totales_desde_items(items)
    execute("""
        UPDATE cotizador_sesiones
        SET total_bruto_nuevo=?, contribucion_nueva=?, margen_nuevo_pct=?,
            costo_total_venta=?, updated_at=?
        WHERE id=?
    """, (
        totals["total_bruto"], totals["contribucion"], totals["margen"],
        totals["costo_neto_total"], now_str(), sesion_id
    ))
    return totals



def cotizador_guardar_items_desde_form(sesion_id):
    """
    Guarda los cambios del tablero tipo Excel y recalcula la sesión.
    Usado por Guardar cambios y Generar PDF y guardar.
    """
    items_actuales = query_all("SELECT * FROM cotizador_items WHERE sesion_id = ? ORDER BY id", (sesion_id,))
    for item in items_actuales:
        iid = item["id"]
        descripcion = request.form.get(f"descripcion_{iid}", item["descripcion"]).strip()
        cantidad = parse_number(request.form.get(f"cantidad_{iid}"), item["cantidad"])
        precio_pdf = parse_number(request.form.get(f"precio_pdf_{iid}"), item["precio_unitario_bruto_pdf"])
        descuento = parse_number(request.form.get(f"descuento_{iid}"), item["descuento_pct"])
        precio_final = parse_number(request.form.get(f"precio_final_{iid}"), item["precio_unitario_bruto_final"])
        costo_neto = parse_number(request.form.get(f"costo_neto_{iid}"), item["costo_neto"])
        observacion = request.form.get(f"observacion_{iid}", item["observacion"] or "").strip()

        if precio_final <= 0 and precio_pdf > 0:
            precio_final = precio_pdf * (1 - descuento / 100)
        if precio_pdf > 0 and precio_final > 0:
            descuento = (1 - precio_final / precio_pdf) * 100

        calc = cotizador_calculos_linea(cantidad, precio_final, costo_neto)
        execute("""
            UPDATE cotizador_items
            SET descripcion=?, cantidad=?, precio_unitario_bruto_pdf=?, descuento_pct=?,
                precio_unitario_bruto_final=?, costo_neto=?, margen_pct=?, contribucion_unitaria=?,
                contribucion_total=?, total_linea_bruto_final=?, observacion=?, updated_at=?
            WHERE id=?
        """, (
            descripcion, cantidad, precio_pdf, descuento, precio_final, costo_neto,
            calc["margen_pct"], calc["contribucion_unitaria"], calc["contribucion_total"],
            calc["total_linea_bruto_final"], observacion, now_str(), iid
        ))

    validez = int(parse_number(request.form.get("validez_oferta_dias"), 2))
    if validez <= 0:
        validez = 2
    if validez > 60:
        validez = 60
    execute("UPDATE cotizador_sesiones SET validez_oferta_dias=?, updated_at=? WHERE id=?", (validez, now_str(), sesion_id))

    totals = cotizador_recalcular_sesion(sesion_id)
    return totals


def cotizador_crear_desde_pdf(file_storage):
    filename = secure_filename(file_storage.filename or "cotizacion.pdf")
    if not filename.lower().endswith(".pdf"):
        raise ValueError("El archivo debe ser PDF.")

    texto_pdf = extraer_texto_pdf_cotizacion(file_storage)
    cabecera = parsear_cabecera_cotizacion_pdf(texto_pdf)
    productos_pdf = parsear_productos_cotizacion_pdf(texto_pdf)
    totales_pdf = parsear_totales_cotizacion_pdf(texto_pdf)
    if not productos_pdf:
        raise ValueError("No se detectaron productos en el PDF. Verifica que sea una cotización con tabla Código / Descripción / Cantidad / Precio Unit. / Valor.")

    # La tabla de Facturación.cl puede traer una rebaja o retención global.
    # Ejemplo real: las líneas suman 6.475.640, pero el total bruto PDF es 6.151.859.
    # Por eso el cotizador debe partir desde el TOTAL PDF, no desde la suma bruta de líneas.
    suma_lineas_pdf = sum(float(p.get("valor_total_linea") or 0) for p in productos_pdf)
    total_bruto_pdf = float(totales_pdf.get("bruto") or 0)
    if total_bruto_pdf <= 0:
        total_bruto_pdf = suma_lineas_pdf

    factor_pdf = (total_bruto_pdf / suma_lineas_pdf) if suma_lineas_pdf > 0 and total_bruto_pdf > 0 else 1.0
    # Evita factores absurdos por PDFs mal parseados.
    if factor_pdf <= 0 or factor_pdf > 2:
        factor_pdf = 1.0

    descuento_pdf_pct = (1 - factor_pdf) * 100

    user = current_user()
    sesion_id = insert_and_get_id("""
        INSERT INTO cotizador_sesiones (
            numero_pdf, cliente, contacto, fecha_pdf, archivo_nombre, texto_pdf,
            total_neto_pdf, iva_pdf, total_bruto_pdf,
            total_bruto_actual, contribucion_actual, margen_actual_pct,
            total_bruto_nuevo, contribucion_nueva, margen_nuevo_pct,
            costo_total_venta, validez_oferta_dias, estado, usuario_id, usuario_nombre, created_at, updated_at
        )
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, 0, 0, 0, 0, 0, 2, 'Borrador', ?, ?, ?, ?)
    """, (
        cabecera.get("numero"), cabecera.get("cliente"), cabecera.get("contacto"), cabecera.get("fecha"),
        filename, texto_pdf, totales_pdf.get("neto"), totales_pdf.get("iva"), total_bruto_pdf,
        user["id"] if user else None, user["full_name"] if user else "Sistema", now_str(), now_str()
    ))

    total_actual = 0.0
    costo_actual_total = 0.0
    contrib_actual_total = 0.0

    running_total_lineas = 0.0
    total_productos_pdf = len(productos_pdf)

    for idx_p, p in enumerate(productos_pdf):
        prod = producto_por_codigo(p["codigo"])
        encontrado = 1 if prod else 0
        costo = float(prod["precio_compra_neto"] or 0) if prod else 0.0
        stock = float(prod["stock"] or 0) if prod else 0.0
        precio_pdf = float(p["precio_unitario_bruto"] or 0)
        cantidad = float(p["cantidad"] or 0)

        # Precio final inicial: precio PDF ajustado proporcionalmente para que el total nuevo
        # parta igual al total bruto oficial del PDF. La última línea absorbe diferencias
        # de redondeo para que el total bruto nuevo coincida con el total del PDF.
        precio_final = precio_pdf * factor_pdf
        if idx_p == total_productos_pdf - 1 and cantidad > 0 and total_bruto_pdf > 0:
            precio_final = max((total_bruto_pdf - running_total_lineas) / cantidad, 0)

        descuento_inicial = (1 - (precio_final / precio_pdf)) * 100 if precio_pdf > 0 else 0.0
        if abs(descuento_inicial) <= 0.01:
            descuento_inicial = 0.0

        if prod:
            obs = "Costo obtenido desde maestra productos"
            if abs(descuento_inicial) > 0.01:
                obs += f" · Ajuste global PDF aplicado: {descuento_inicial:.2f}%"
        else:
            obs = "Código no encontrado en maestra; ingresar costo manual."
            if abs(descuento_inicial) > 0.01:
                obs += f" Ajuste global PDF aplicado: {descuento_inicial:.2f}%."

        calc = cotizador_calculos_linea(cantidad, precio_final, costo)

        execute("""
            INSERT INTO cotizador_items (
                sesion_id, codigo, descripcion, cantidad, precio_unitario_bruto_pdf,
                valor_total_linea_pdf, descuento_pct, precio_unitario_bruto_final,
                costo_neto, stock, encontrado, margen_pct, contribucion_unitaria,
                contribucion_total, total_linea_bruto_final, observacion, created_at, updated_at
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            sesion_id, p["codigo"], p["descripcion"], cantidad, precio_pdf,
            p["valor_total_linea"], descuento_inicial, precio_final, costo, stock, encontrado,
            calc["margen_pct"], calc["contribucion_unitaria"], calc["contribucion_total"],
            calc["total_linea_bruto_final"], obs, now_str(), now_str()
        ))

        total_linea_final = precio_final * cantidad
        running_total_lineas += total_linea_final
        total_actual += total_linea_final
        costo_actual_total += costo * cantidad
        contrib_actual_total += ((precio_final / (1 + iva_rate())) - costo) * cantidad

    margen_actual = (total_actual / (costo_actual_total * (1 + iva_rate())) - 1) if costo_actual_total > 0 else 0.0

    # Por redondeos unitarios puede quedar diferencia de pesos; se fuerza el total de sesión
    # a coincidir con el PDF cuando hay total PDF válido. Los ítems siguen mostrando el ajuste.
    total_sesion = total_bruto_pdf if total_bruto_pdf > 0 else total_actual
    if total_actual > 0 and abs(total_sesion - total_actual) > 2:
        # Mantiene contribución coherente con el total objetivo recalculando contra costo total.
        contrib_actual_total = (total_sesion / (1 + iva_rate())) - costo_actual_total
        margen_actual = (total_sesion / (costo_actual_total * (1 + iva_rate())) - 1) if costo_actual_total > 0 else 0.0

    execute("""
        UPDATE cotizador_sesiones
        SET total_bruto_actual=?, contribucion_actual=?, margen_actual_pct=?,
            total_bruto_nuevo=?, contribucion_nueva=?, margen_nuevo_pct=?,
            costo_total_venta=?, updated_at=?
        WHERE id=?
    """, (
        total_sesion, contrib_actual_total, margen_actual,
        total_sesion, contrib_actual_total, margen_actual,
        costo_actual_total, now_str(), sesion_id
    ))

    write_audit("cotizador", "importar_pdf", sesion_id, "archivo", "", filename)
    return sesion_id


@app.route("/cotizador", methods=["GET", "POST"])
@login_required
@permission_any_required("ventas", "cotizador")
def cotizador():
    if request.method == "POST":
        archivo = request.files.get("pdf_cotizacion")
        if not archivo or not archivo.filename:
            flash("Debes seleccionar un PDF de cotización.", "error")
            return redirect(url_for("cotizador"))
        try:
            sesion_id = cotizador_crear_desde_pdf(archivo)
            flash("PDF importado correctamente. Revisa productos, costos, descuentos y margen.", "success")
            return redirect(url_for("cotizador_detalle", sesion_id=sesion_id))
        except Exception as exc:
            flash(f"No se pudo procesar el PDF: {exc}", "error")
            return redirect(url_for("cotizador"))

    # Mantiene actualizado el costo total visible en Últimos análisis.
    try:
        execute("""
            UPDATE cotizador_sesiones
            SET costo_total_venta = (
                SELECT COALESCE(SUM(COALESCE(cantidad,0) * COALESCE(costo_neto,0)), 0)
                FROM cotizador_items
                WHERE cotizador_items.sesion_id = cotizador_sesiones.id
            )
        """)
    except Exception:
        pass

    recientes = query_all("SELECT * FROM cotizador_sesiones ORDER BY id DESC LIMIT 20")
    last_import = query_one("SELECT * FROM producto_importaciones ORDER BY id DESC LIMIT 1")
    return render_page("Cotizador", r"""
    <div class="page-head">
        <div>
            <h1>Cotizador</h1>
            <p>Importa una cotización PDF, cruza costos con la maestra y recalcula descuentos, margen y contribución.</p>
        </div>
    </div>

    <div class="grid grid-3">
        <div class="stat"><span>Última maestra productos</span><strong style="font-size:15px;">{% if last_import %}{{ last_import.created_at }}{% else %}Sin importar{% endif %}</strong></div>
        <div class="stat"><span>Cotizaciones importadas</span><strong>{{ recientes|length }}</strong></div>
        <div class="stat"><span>IVA configurado</span><strong>{{ iva|percent }}</strong></div>
    </div>

    <div class="card" style="margin-top:18px;">
        <h2>Importar PDF de cotización</h2>
        <form method="post" enctype="multipart/form-data">
            <div class="form-row-2">
                <div>
                    <label>PDF de cotización</label>
                    <input type="file" name="pdf_cotizacion" accept="application/pdf" required>
                </div>
                <div class="actions" style="margin-top:24px;">
                    <button class="btn btn-primary">Leer PDF y abrir cotizador</button>
                    {% if has_perm('productos') %}<a class="btn btn-secondary" href="{{ url_for('productos') }}">Actualizar maestra productos</a>{% endif %}
                </div>
            </div>
        </form>
        <div class="placeholder" style="margin-top:14px;">
            El PDF debe tener columnas: Código, Descripción, Cantidad, Precio Unit. y Valor. El costo neto se obtiene desde la maestra por Código Producto.
        </div>
    </div>
    ...
    <div class="card">
        <div class="section-title">
            <div>
                ...
                <p class="muted">Vista compacta con valores actuales y valores nuevos para revisar rápidamente la rentabilidad.</p>
            </div>
            {% if is_superior %}
            <span class="badge warn">Superior: eliminación habilitada solo para gus</span>
            {% endif %}
        </div>
        <div class="table-wrap">
            <table class="cotizador-analisis-table">
                <colgroup>
                ...
                <tr>
                    <td>{{ c.id }}</td>
                    <td><b>{{ c.numero_pdf }}</b></td>
                    <td class="truncate" title="{{ c.cliente }}">{{ c.cliente }}</td>
                    <td class="truncate" title="{{ c.usuario_nombre }}">{{ c.usuario_nombre }}</td>
                    <td class="truncate" title="{{ c.contacto }}">{{ c.contacto }}</td>
                    <td class="num">{{ c.total_bruto_actual|money }}</td>
                    <td class="num">{{ c.contribucion_actual|money }}</td>
                    <td class="num">{{ c.margen_actual_pct|percent }}</td>
                    <td class="num">{{ c.total_bruto_nuevo|money }}</td>
                    <td class="num">{{ c.contribucion_nueva|money }}</td>
                    <td class="num">{{ c.margen_nuevo_pct|percent }}</td>
                    <td class="num">{{ c.costo_total_venta|gross_cost|money }}</td>
                    <td>{{ c.created_at }}</td>
                    <td>
                        <div class="actions" style="margin:0;">
                            <a class="btn btn-secondary btn-small" href="{{ url_for('cotizador_detalle', sesion_id=c.id) }}">Abrir</a>
                            <a class="btn btn-primary btn-small" href="{{ url_for('cotizador_pdf', sesion_id=c.id) }}">PDF</a>
                            {% if is_superior %}
                            <form method="post" action="{{ url_for('cotizador_eliminar', sesion_id=c.id) }}" style="display:inline;" onsubmit="return confirm('¿Eliminar este análisis de cotización?');">
                ...
                {% else %}
                <tr><td colspan="14" class="muted">Sin cotizaciones importadas.</td></tr>
                {% endfor %}
            </table>
        </div>
    </div>
    """, recientes=recientes, last_import=last_import, iva=iva_rate(), has_perm=has_perm, is_admin=is_admin(), is_superior=is_superior())


@app.route("/cotizador/<int:sesion_id>", methods=["GET", "POST"])
@login_required
@permission_any_required("ventas", "cotizador")
def cotizador_detalle(sesion_id):
    ses = query_one("SELECT * FROM cotizador_sesiones WHERE id = ?", (sesion_id,))
    if not ses:
        flash("Cotización importada no encontrada.", "error")
        return redirect(url_for("cotizador"))

    if request.method == "POST":
        action = request.form.get("action", "guardar")
        cotizador_guardar_items_desde_form(sesion_id)
        write_audit("cotizador", "guardar_cambios", sesion_id, "items", "", "precios/costos actualizados")

        if action == "pdf_guardar":
            flash("Cambios guardados. Generando PDF.", "success")
            return redirect(url_for("cotizador_pdf", sesion_id=sesion_id))

        flash("Cambios guardados y recalculados.", "success")
        return redirect(url_for("cotizador_detalle", sesion_id=sesion_id))

    cotizador_recalcular_sesion(sesion_id)
    ses = query_one("SELECT * FROM cotizador_sesiones WHERE id = ?", (sesion_id,))
    items = query_all("SELECT * FROM cotizador_items WHERE sesion_id = ? ORDER BY id", (sesion_id,))
    faltantes = query_one("SELECT COUNT(*) c FROM cotizador_items WHERE sesion_id=? AND encontrado=0", (sesion_id,))["c"]
    suma_lineas_pdf = sum(float(i["valor_total_linea_pdf"] or 0) for i in items)
    ajuste_pdf_pct = (1 - (float(ses["total_bruto_pdf"] or 0) / suma_lineas_pdf)) * 100 if suma_lineas_pdf > 0 and float(ses["total_bruto_pdf"] or 0) > 0 else 0

    return render_page("Cotizador detalle", r"""
    <div class="page-head">
        <div>
            <h1>Cotizador PDF N° {{ ses.numero_pdf or ses.id }}</h1>
            <p>{{ ses.cliente or 'Cliente no identificado' }} · Contacto: {{ ses.contacto or '-' }} · PDF: {{ ses.archivo_nombre }}</p>
        </div>
        <div class="actions">
            <a class="btn btn-secondary" href="{{ url_for('cotizador') }}">Volver</a>
            <a class="btn btn-primary" href="{{ url_for('cotizador_pdf', sesion_id=ses.id) }}">Generar PDF</a>
            <a class="btn btn-secondary" href="{{ url_for('cotizador_excel', sesion_id=ses.id) }}">Exportar Excel</a>
            {% if is_superior %}
            <form method="post" action="{{ url_for('cotizador_eliminar', sesion_id=ses.id) }}" style="display:inline;" onsubmit="return confirm('¿Eliminar este análisis de cotización? Esta acción no se puede deshacer.');">
    ...
    {% if faltantes %}
    <div class="flash error">Hay {{ faltantes }} producto(s) sin costo desde la maestra. Ingresa costo neto manual o actualiza la maestra de productos.</div>
    {% endif %}

    {% if ajuste_pdf_abs > 0.01 %}
    <div class="flash info">
        Se detectó ajuste global del PDF de {{ "%.2f"|format(ajuste_pdf_pct) }}%.
        Por eso el cotizador parte con el total bruto nuevo igual al total bruto oficial del PDF, no con la suma simple de líneas.
    </div>
    {% endif %}

    <div class="grid grid-4">
        <div class="stat"><span>PDF total bruto</span><strong>{{ ses.total_bruto_pdf|money }}</strong></div>
        <div class="stat"><span>Total bruto nuevo</span><strong id="totalBrutoCard">{{ ses.total_bruto_nuevo|money }}</strong></div>
        <div class="stat"><span>Contribución nueva</span><strong id="contribNueva">{{ ses.contribucion_nueva|money }}</strong></div>
        <div class="stat"><span>Costo total bruto</span><strong id="costoTotalVenta">{{ ses.costo_total_venta|gross_cost|money }}</strong></div>
        <div class="stat"><span>Margen actual</span><strong>{{ ses.margen_actual_pct|percent }}</strong></div>
        <div class="stat"><span>Contribución actual</span><strong>{{ ses.contribucion_actual|money }}</strong></div>
        <div class="stat"><span>Margen nuevo</span><strong id="margenNuevo">{{ ses.margen_nuevo_pct|percent }}</strong></div>
    </div>

    <div class="card" style="margin-top:18px;">
        <h2>Herramientas de precio</h2>
        <p class="muted">
            Usa estas herramientas para aplicar descuento, fijar una venta bruta final o definir una contribución total objetivo.
            Luego presiona <b>Guardar cambios</b> para dejarlo registrado.
        </p>
        <div class="calc-toolbar">
            <div>
                <label>Descuento general (%)</label>
                <input id="descuentoGlobal" type="number" step="0.01" placeholder="Ej: 5">
            </div>
            <div>
                <label>Venta bruta final objetivo ($)</label>
                <input id="ventaBrutaObjetivo" type="number" step="1" placeholder="Ej: 6000000">
            </div>
            <div>
                <label>Contribución total objetivo ($)</label>
                <input id="contribucionObjetivo" type="number" step="1" placeholder="Ej: 1200000">
            </div>
            <div>
                <label>Total bruto nuevo calculado</label>
                <input id="totalBrutoNuevo" disabled>
            </div>
            <div>
                <label>Validez oferta (días)</label>
                <input name="validez_oferta_dias" form="cotizadorForm" type="number" min="1" max="60" step="1" value="{{ ses.validez_oferta_dias or 2 }}">
            </div>
        </div>
        <div class="actions" style="margin-top:14px;">
            <button type="button" class="btn btn-secondary" onclick="aplicarDescuentoGlobal()">Aplicar descuento</button>
            <button type="button" class="btn btn-secondary" onclick="fijarVentaBruta()">Fijar total bruto</button>
            <button type="button" class="btn btn-secondary" onclick="fijarContribucion()">Fijar contribución</button>
            <button type="button" class="btn btn-primary" onclick="recalcularTodo()">Calcular</button>
        </div>
    </div>

    <form method="post" id="cotizadorForm">
        <div class="excel-board">
            <table class="excel-table" id="tablaCotizador">
                <thead>
                    <tr>
                        <th>Estado</th>
                        <th>Código</th>
                        ...
                        <th>PV bruto PDF</th>
                        <th>Descuento %</th>
                        <th>PV bruto final</th>
                        <th>Costo neto</th>
                        <th>Stock</th>
                        <th>Margen producto</th>
                        <th>Contrib. unitaria</th>
                        <th>Contrib. total línea</th>
                        <th>Total línea bruto</th>
                        <th>Observación</th>
                    </tr>
                </thead>
                <tbody>
                    {% for i in items %}
                    <tr data-row="1">
                        <td>{% if i.encontrado %}<span class="cotizador-badge ok">OK</span>{% else %}<span class="cotizador-badge bad">Sin costo</span>{% endif %}</td>
                        <td><b>{{ i.codigo }}</b></td>
                        <td><textarea class="excel-desc" name="descripcion_{{ i.id }}">{{ i.descripcion }}</textarea></td>
                        <td><input class="qty" name="cantidad_{{ i.id }}" type="number" step="0.01" value="{{ '%.2f'|format(i.cantidad or 0) }}"></td>
                        <td><input class="pdf-price" name="precio_pdf_{{ i.id }}" type="number" step="0.01" value="{{ '%.2f'|format(i.precio_unitario_bruto_pdf or 0) }}"></td>
                        <td><input class="discount" name="descuento_{{ i.id }}" type="number" step="0.01" value="{{ '%.2f'|format(i.descuento_pct or 0) }}"></td>
                        <td><input class="final-price" name="precio_final_{{ i.id }}" type="number" step="0.01" value="{{ '%.2f'|format(i.precio_unitario_bruto_final or 0) }}"></td>
                        <td><input class="cost" name="costo_neto_{{ i.id }}" type="number" step="0.01" value="{{ '%.2f'|format(i.costo_neto or 0) }}"></td>
                        <td>{{ '%.2f'|format(i.stock or 0) }}</td>
                        <td class="margin-cell">{{ i.margen_pct|percent }}</td>
                        <td class="contrib-unit-cell">{{ i.contribucion_unitaria|money }}</td>
                        <td class="contrib-total-cell">{{ i.contribucion_total|money }}</td>
                        <td class="total-line-cell">{{ i.total_linea_bruto_final|money }}</td>
                        <td><input name="observacion_{{ i.id }}" value="{{ i.observacion }}"></td>
                    </tr>
                    {% endfor %}
                </tbody>
            </table>
        ...
        <div class="actions" style="margin-top:18px;">
            ...
            <button type="button" class="btn btn-secondary" onclick="recalcularTodo()">Calcular sin guardar</button>
            <a class="btn btn-secondary" href="{{ url_for('cotizador_pdf', sesion_id=ses.id) }}">Generar PDF</a>
            <button class="btn btn-warning" name="action" value="pdf_guardar">Generar PDF y guardar</button>
        </div>
    </form>

    <div class="grid grid-3" style="margin-top:18px;">
        <div class="quote-total"><span class="muted">PDF Neto</span><h2>{{ ses.total_neto_pdf|money }}</h2></div>
        <div class="quote-total"><span class="muted">PDF IVA</span><h2>{{ ses.iva_pdf|money }}</h2></div>
        <div class="quote-total"><span class="muted">PDF Total bruto</span><h2>{{ ses.total_bruto_pdf|money }}</h2></div>
    </div>

    <script>
        const IVA = {{ iva }};
        function num(v){ const n = parseFloat(String(v || '0').replace(/\\./g, '').replace(',', '.')); return isNaN(n) ? 0 : n; }
        function numInput(v){ const n = parseFloat(String(v || '0').replace(',', '.')); return isNaN(n) ? 0 : n; }
        function clp(v){ return '$' + Math.round(v || 0).toLocaleString('es-CL'); }
        function pct(v){ return ((v || 0) * 100).toLocaleString('es-CL', {minimumFractionDigits:1, maximumFractionDigits:1}) + '%'; }
        function rows(){ return Array.from(document.querySelectorAll('#tablaCotizador tbody tr')); }

        function recalcRow(tr, source){
            const qty = numInput(tr.querySelector('.qty').value);
            const pdf = numInput(tr.querySelector('.pdf-price').value);
            const discInput = tr.querySelector('.discount');
            const finalInput = tr.querySelector('.final-price');
            const cost = numInput(tr.querySelector('.cost').value);
            let discount = numInput(discInput.value);
            let finalPrice = numInput(finalInput.value);

            if(source === 'discount'){
                finalPrice = pdf * (1 - discount / 100);
                finalInput.value = finalPrice.toFixed(2);
            } else if(source === 'price'){
                discount = pdf > 0 ? (1 - finalPrice / pdf) * 100 : 0;
                discInput.value = discount.toFixed(2);
            }

            const ventaNeta = finalPrice / (1 + IVA);
            const contribUnit = ventaNeta - cost;
            const contribTotal = contribUnit * qty;
            const totalLine = finalPrice * qty;
            const costTotal = cost * qty;
            const margin = cost > 0 ? (finalPrice / (cost * (1 + IVA)) - 1) : 0;

            tr.querySelector('.margin-cell').textContent = pct(margin);
            tr.querySelector('.contrib-unit-cell').textContent = clp(contribUnit);
            tr.querySelector('.contrib-total-cell').textContent = clp(contribTotal);
            tr.querySelector('.total-line-cell').textContent = clp(totalLine);
            return {totalLine, contribTotal, costTotal};
        }

        function totalsActuales(){
            let total = 0, contrib = 0, costTotal = 0;
            rows().forEach(tr => {
                const r = recalcRow(tr);
                total += r.totalLine;
                contrib += r.contribTotal;
                costTotal += r.costTotal;
            });
            const margin = costTotal > 0 ? (total / (costTotal * (1 + IVA)) - 1) : 0;
            return {total, contrib, costTotal, margin};
        }

        function pintarTotales(t){
            document.getElementById('totalBrutoNuevo').value = clp(t.total);
            document.getElementById('totalBrutoCard').textContent = clp(t.total);
            document.getElementById('margenNuevo').textContent = pct(t.margin);
            document.getElementById('contribNueva').textContent = clp(t.contrib);
            const costoEl = document.getElementById('costoTotalVenta');
            if (costoEl) { costoEl.textContent = clp(t.costTotal * (1 + IVA)); }
        }

        function recalcularTodo(){
            pintarTotales(totalsActuales());
        }

        function aplicarDescuentoGlobal(){
            const d = numInput(document.getElementById('descuentoGlobal').value);
            rows().forEach(tr => {
                tr.querySelector('.discount').value = d.toFixed(2);
                recalcRow(tr, 'discount');
            });
            recalcularTodo();
        }

        function fijarVentaBruta(){
            const target = numInput(document.getElementById('ventaBrutaObjetivo').value);
            if(target <= 0){ alert('Ingresa una venta bruta final objetivo válida.'); return; }
            setVentaBrutaObjetivo(target);
        }

        function setVentaBrutaObjetivo(target){
            const current = totalsActuales().total;
            if(current <= 0){ alert('No hay total actual para ajustar.'); return; }
            const factor = target / current;
            rows().forEach(tr => {
                const finalInput = tr.querySelector('.final-price');
                const pdf = numInput(tr.querySelector('.pdf-price').value);
                let newPrice = numInput(finalInput.value) * factor;
                finalInput.value = newPrice.toFixed(2);
                const discount = pdf > 0 ? (1 - newPrice / pdf) * 100 : 0;
                tr.querySelector('.discount').value = discount.toFixed(2);
                recalcRow(tr, 'price');
            });

            // Ajusta la última línea para absorber diferencia de redondeo.
            const diff = target - totalsActuales().total;
            const allRows = rows();
            if(Math.abs(diff) >= 1 && allRows.length){
                const last = allRows[allRows.length - 1];
                const qty = numInput(last.querySelector('.qty').value);
                if(qty > 0){
                    const input = last.querySelector('.final-price');
                    input.value = (numInput(input.value) + diff / qty).toFixed(2);
                    recalcRow(last, 'price');
                }
            }
            recalcularTodo();
        }

        function fijarContribucion(){
            const target = numInput(document.getElementById('contribucionObjetivo').value);
            const t = totalsActuales();
            const requiredGross = (target + t.costTotal) * (1 + IVA);
            if(requiredGross <= 0){ alert('No se puede calcular venta bruta con esa contribución objetivo.'); return; }
            setVentaBrutaObjetivo(requiredGross);
        }

        rows().forEach(tr => {
            tr.querySelectorAll('input').forEach(inp => inp.addEventListener('input', function(){
                if(this.classList.contains('discount')) recalcRow(tr, 'discount');
                else if(this.classList.contains('final-price')) recalcRow(tr, 'price');
                else recalcRow(tr);
                recalcularTodo();
            }));
        });
    ...
    """, ses=ses, items=items, faltantes=faltantes, iva=iva_rate(), suma_lineas_pdf=suma_lineas_pdf, ajuste_pdf_pct=ajuste_pdf_pct, ajuste_pdf_abs=abs(ajuste_pdf_pct), is_admin=is_admin(), is_superior=is_superior())



@app.route("/cotizador/<int:sesion_id>/pdf")
@login_required
@permission_any_required("ventas", "cotizador", "cotizador_exportar")
def cotizador_pdf(sesion_id):
    ses = query_one("SELECT * FROM cotizador_sesiones WHERE id = ?", (sesion_id,))
    if not ses:
        flash("Cotización importada no encontrada.", "error")
        return redirect(url_for("cotizador"))

    # Asegura totales actualizados antes de generar PDF.
    cotizador_recalcular_sesion(sesion_id)
    ses = query_one("SELECT * FROM cotizador_sesiones WHERE id = ?", (sesion_id,))
    items = query_all("SELECT * FROM cotizador_items WHERE sesion_id = ? ORDER BY id", (sesion_id,))
    ...
    try:
        ...
        flash(f"No se pudo generar PDF. Falta dependencia ReportLab: {exc}", "error")
        return redirect(url_for("cotizador_detalle", sesion_id=sesion_id))
    ...
    total_bruto = float(ses["total_bruto_nuevo"] or 0)
    neto = total_bruto / (1 + iva_rate()) if total_bruto else 0
    iva_val = total_bruto - neto
    ...
    title = ParagraphStyle("CotizadorTitle", parent=styles["Title"], fontSize=18, leading=22, alignment=0, textColor=colors.HexColor("#0f172a"))
    small = ParagraphStyle("CotizadorSmall", parent=styles["Normal"], fontSize=8, leading=10, textColor=colors.HexColor("#334155"))
    normal = ParagraphStyle("CotizadorNormal", parent=styles["Normal"], fontSize=9, leading=11)

    story = []

    logo_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "static", "ferreteria_san_pedro_logo.jpg")
    if os.path.exists(logo_path):
        logo = Image(logo_path, width=5.0*cm, height=1.58*cm)
    else:
        logo = Paragraph("Ferreteria San Pedro", title)

    brand_block = [
        Paragraph("<b>Ferreteria San Pedro</b>", title),
        Paragraph("Socom Ruz Spa · R.U.T. 77.351.584-0", small),
        Paragraph("Cotización comercial generada desde Ferretería Cloud Tool", small),
    ]
    brand_table = Table([[logo, brand_block]], colWidths=[5.7*cm, 12.0*cm])
    ...
    contact_data = [[
        Paragraph("<b>Visítanos en:</b><br/>Sucursal 1: Av. 21 de Mayo 73, Quillota<br/>Sucursal 2: Av. 21 de Mayo 420, Quillota", small),
        Paragraph("<b>WhatsApp:</b><br/>+56 9 3563 0950<br/>+56 9 4907 3615", small),
    ]]
    contact_table = Table(contact_data, colWidths=[11.8*cm, 5.9*cm])
    ...
    story.append(Paragraph("COTIZACIÓN", ParagraphStyle("CotTitle2", parent=styles["Heading2"], fontSize=14, textColor=colors.HexColor("#0f766e"))))
    story.append(Spacer(1, 8))

    numero = ses["numero_pdf"] or str(ses["id"])
    header = [
        ["N° Cotización", numero, "Fecha PDF", ses["fecha_pdf"] or date_short(ses["created_at"])],
        ["Cliente", ses["cliente"] or "Cliente no identificado", "Contacto", ses["contacto"] or ""],
        ["PDF origen", ses["archivo_nombre"] or "", "Vendedor", ses["usuario_nombre"] or ""],
    ]
    header_table = Table(header, colWidths=[3.0*cm, 6.2*cm, 2.6*cm, 6.0*cm])
    ...
    for i in items:
        cantidad = float(i["cantidad"] or 0)
        precio = float(i["precio_unitario_bruto_final"] or 0)
        total_linea = float(i["total_linea_bruto_final"] or (cantidad * precio))
        data.append([
            str(i["codigo"] or ""),
            Paragraph(xml_escape(str(i["descripcion"] or "")), small),
            fmt_qty(cantidad),
            format_clp(precio),
            format_clp(total_linea),
        ])

    if len(data) == 1:
        data.append(["", "Sin productos", "", "", ""])

    table = Table(data, colWidths=[3.0*cm, 8.4*cm, 2.0*cm, 2.8*cm, 2.8*cm], repeatRows=1)
    ...
    totals = Table([
        ["Neto", format_clp(neto)],
        [f"IVA {round(iva_rate()*100, 2)}%", format_clp(iva_val)],
        ["Total", format_clp(total_bruto)],
    ...
    validez = int(ses["validez_oferta_dias"] or 2)
    condiciones = (
        "CONDICIONES COMERCIALES<br/>"
        f"1.- Validez de la oferta: {validez} dias.<br/>"
        "2.- Plazo de entrega: sujeto a disponibilidad de stock."
    )
    story.append(Paragraph(condiciones, normal))
    story.append(Spacer(1, 8))
    story.append(Paragraph("Ferretería San Pedro · Av. 21 de Mayo 73 y Av. 21 de Mayo 420, Quillota · WhatsApp +56 9 3563 0950 / +56 9 4907 3615", small))
    ...
    return send_file(
        buffer,
        as_attachment=True,
        download_name=f"cotizador_{numero}_{today_str()}.pdf",
        mimetype="application/pdf"
    )


@app.route("/cotizador/<int:sesion_id>/eliminar", methods=["POST"])
@login_required
def cotizador_eliminar(sesion_id):
    if not is_superior():
        flash("Solo el usuario superior gus puede eliminar análisis del cotizador.", "error")
        return redirect(url_for("cotizador"))
    ...
    numero = ses["numero_pdf"] or str(sesion_id)
    execute("DELETE FROM cotizador_items WHERE sesion_id = ?", (sesion_id,))
    execute("DELETE FROM cotizador_sesiones WHERE id = ?", (sesion_id,))
    write_audit("cotizador", "eliminar", sesion_id, "numero_pdf", numero, "eliminado")
    flash(f"Análisis de cotización {numero} eliminado.", "success")
    return redirect(url_for("cotizador"))



@app.route("/cotizador/<int:sesion_id>/excel")
@login_required
@permission_any_required("ventas", "cotizador", "cotizador_exportar")
def cotizador_excel(sesion_id):
    ses = query_one("SELECT * FROM cotizador_sesiones WHERE id = ?", (sesion_id,))
    if not ses:
        flash("Cotización importada no encontrada.", "error")
        return redirect(url_for("cotizador"))
    ...
    ws = wb.active
    ws.title = "Cotizador"
    ws.append(["Cotizador PDF", ses["numero_pdf"], "Cliente", ses["cliente"], "Contacto", ses["contacto"]])
    ws.append([])
    ws.append([
        "Código", "Descripción", "Cantidad", "PV bruto PDF", "Descuento %", "PV bruto final",
        "Costo neto", "Stock", "Encontrado", "Margen producto", "Contribución unitaria",
        "Contribución total línea", "Total línea bruto", "Observación"
    ])
    for i in items:
        ws.append([
            i["codigo"], i["descripcion"], i["cantidad"], i["precio_unitario_bruto_pdf"],
            i["descuento_pct"], i["precio_unitario_bruto_final"], i["costo_neto"], i["stock"],
            "SI" if i["encontrado"] else "NO", i["margen_pct"], i["contribucion_unitaria"],
            i["contribucion_total"], i["total_linea_bruto_final"], i["observacion"]
        ])
    output = BytesIO()
    wb.save(output)
    output.seek(0)
    return send_file(
        output,
        as_attachment=True,
        download_name=f"cotizador_{ses['numero_pdf'] or sesion_id}_{today_str()}.xlsx",
...
@login_required
@permission_any_required("productos", "productos_importar")
def productos():
    if request.method == "POST":
